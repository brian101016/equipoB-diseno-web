...
const _StudentScreen = (props: StudentScreenProps) => {
  ...
  const activityOptions = listData.students.flatMap((student) => student.activities.map((activity) => activity.nameActivity)).filter((name, index, self) => self.indexOf(name) === index);
  ...
  const filitro = listData.students.map((student) => {
    const filitroActivities = student.activities.filter((activity) => {
      return selectActivity === "Actividad" || activity.nameActivity === selectActivity;
    });
  ...
  return (
    <div className={props.className}>
      ...
      <div className="justifySelect">
          ...
          {openName && (
            <div className="optionList">
              {nameOptions.map((option, index) => (
                <div className="optionItem" key={index} onClick={() => selectOptionName(option)}>
                  {option}
                </div>
              ))}
            </div>
        ...
        <button className="limpiarLaslistas" onClick={resetSelect}>Todas las listas</button>
        <div className="customSelect">
          <div className="selectHeader" onClick={ActivityDropDown}>
            <div className="selectedOption">{selectActivity}</div>
            <div className={!openActivity ? "arrowIcon" : "arrowIcon active"}></div>
          </div>
          {openActivity && (
            <div className="optionList">
              {activityOptions.map((option, index) => (
                <div className="optionItem" key={index} onClick={() => selectOptionActivity(option)}>
                  {option}
                </div>
              ))}
            </div>
      ...
      <div className="listBox">
        {filitro.map((student) => (
          student.activities.map((activity) => (
            <div
              className={`textoStyle`}
              key={activity.id}
            >
              <div className="listNameAct" onClick={() => menuDesplz(student.id, activity.id)}>
                <div className="arrowText">
                  <div
                    className={`arrowIconIzq ${student.id === selectNameId && activity.id === selectedActivityId ? 'active' : ''}`}
                  ></div>
              ...
              <div className="Date">
                ...
                <a className="iconCalificar"><img src={mini_icono_calificar}/></a>
              </div>
              ...
              <div className={`allListLinea ${student.id === selectNameId && activity.id === selectedActivityId ? 'despzList' : ''}`}>
              </div>
            </div>
          ))
        ))}
